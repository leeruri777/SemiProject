...
<div class="container p-5" >	
	...
	<%-- 배송 주소록 관리 표 --%>
	<form name="updateAddressFrm">
	<input type="hidden" name="userid" value="${sessionScope.loginuser.userid}"/>
	<input type="hidden" name="ano" value="${addressVo.ano}"/>
		
		<table class="table  motable">	 
		    <tr>
		      <td>배송지명 *</td>
		      <td><input id='delivername' name="delivername" maxlength = "15" class="form-control col-sm-3" type='text' value = "${addressVo.delivername}"/></td>
		    </tr>
			<tr>
		      <td>성명 *</td>
		      <td><input id='name' name="name" maxlength ="10" class="form-control col-sm-3" type='text' value = "${addressVo.name}"/></td>
		    </tr>
			<tr>
		      <td>주소 *</td>
		      <td>
	
		      <div class="form-inline pb-1"><input id="postcode" name="postcode"  class="form-control col-sm-2 required" type='text' value = "${addressVo.postcode}" />&nbsp;&nbsp;
		      	<button type="button" onclick="execDaumPostcode()" class="btn btn-light" style="font-size: 9pt; padding: 3px 8px;">우편번호</button>
		      </div>
		      <div class="form-inline pb-1"><input id="address" name="address" class="form-control col-sm-4 required" type='text' placeholder="기본주소(필수)" value = "${addressVo.address}"/></div>
		      <div class="form-inline pb-1">
			      <input id="detailaddress" name="detailaddress" class="form-control col-sm-4" type='text' placeholder="상세주소" value = "${addressVo.detailaddress}"/> 
			      <input type="text" class="form-control col-sm-4" id="extraaddress" name="extraaddress" placeholder="참고항목" value = "${addressVo.extraaddress}"/>
		      </div>
		    ...
		    <tr>
		      <td>휴대전화 *</td>
			   ...
			   &nbsp;-&nbsp;
			      <input id="mo2" name="mo2" maxlength = "4" class="form-control col-sm-1 required" type='text' value="${fn:substring(addressVo.mobile, 3, 7)}" />&nbsp;-&nbsp;
			      <input id="mo3" name="mo3" maxlength = "4" class="form-control col-sm-1 required" type='text' value="${fn:substring(addressVo.mobile, 7, 11)}" />
		      </td>
		    </tr>
		...
		<div class="form-row float-right">
			...
			<input type="hidden" id="default_yn_hidden" name="default_yn" value='n'/>
      		<button type="button" onclick = "updateAddress();" class="btn btn-dark" style="font-size: 11pt;">수정하기</button>
      		&nbsp; &nbsp;
...
$(function(){
	if("${addressVo.mobile}" != ""){
		$("#mo1").val("${fn:substring(addressVo.mobile, 0, 3)}");
	}
	if(${addressVo.default_yn == 'y'}){
		$("input[name=default_yn]").prop("checked", true);
	}
}); 
function updateAddress(){
	
	var frm = document.updateAddressFrm;
	frm.method = "POST";
	frm.action = "/mypage/deliverModify.go";
